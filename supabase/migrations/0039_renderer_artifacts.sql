...
begin
  ...
  if r.id is null or r.status<>'READY' or not r.is_current or f.current_artifact_id is distinct from r.id then raise exception 'document is not downloadable' using errcode='55000'; end if;
...
begin
  ...
  if b.id is null or r.id is null or r.status<>'READY' or not r.is_current or b.current_artifact_id is distinct from r.id or b.status not in ('GENERATED','DOWNLOADED','IMPORT_FAILED','IMPORT_CONFIRMED') then raise exception 'ERP artifact is not downloadable' using errcode='55000'; end if;
...
revoke all on function public.get_document_status(uuid,text), public.download_document(uuid), public.get_erp_artifact_status(uuid,text), public.download_erp_artifact(uuid,uuid) from public, anon;
grant execute on function public.get_document_status(uuid,text), public.download_document(uuid), public.get_erp_artifact_status(uuid,text), public.download_erp_artifact(uuid,uuid) to authenticated;
