...
`0067_account_admin_profile_and_auth_audit.sql` 補齊帳號基本資料與 Auth Admin 安全事件稽核；`0068_account_login_and_bulk_roles.sql` 將登入帳號與選填聯絡 Email 分離，新增唯一 `login_name`，並讓建立帳號與多角色指派在同一資料庫交易完成。既有 Email 登入在管理員指派登入帳號前仍可相容使用。
