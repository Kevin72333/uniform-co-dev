...
9. **共用管理清單**：參考 SPSV29 的大型清單操作後，以 `src/app/ManagementCatalogTable.tsx` 集中商品、組織、員工、兩倉庫存、營運報表與採購差異原因碼的欄位顯示、密度、每頁筆數、資料範圍、排序表頭與首末頁導覽；純 page／column 規則在 `src/domain/management-catalog.ts`，庫存及原因碼的篩選排序分別在自己的 domain module。資料查詢、RLS、匯出稽核與 mutation 仍留在各業務 module；沒有受保護 bulk RPC 前，不得加入批次刪除／停用。維護規則見 `docs/architecture/management-catalog.md`。
...
- 管理清單 seam：`ManagementCatalogTable` 只接收已篩選排序的 rows、欄位定義及 page／sort callbacks；呼叫端保留查詢與 mutation。`management-catalog.ts` 的純 interface 必須固定 page clamp、核准 page size、固定欄位與至少一欄可見。兩倉庫存只讀 `v_item_availability`；營運報表只讀 security-invoker views；原因碼保存只走 `maintain_procurement_difference_reason`。
