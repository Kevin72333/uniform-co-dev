...
Repository-local 最新完整驗證為：69 個 test files、279/279 tests 通過，`npm run lint`、`npm run typecheck`、`npm run build` 與 System Guide parser tests 全部成功；`git diff --check` 沒有內容錯誤。推送後仍需補正式站 read-only UI smoke。下一位 agent 不應把「找不到新的本機 TODO」或本機驗證成功解讀為正式上線已完成。
...
- 管理清單 seam：`ManagementCatalogTable` 只接收已篩選排序的 rows、欄位定義及 page／sort callbacks；呼叫端保留查詢與 mutation。`management-catalog.ts` 的純 interface 必須固定 page clamp、核准 page size、固定欄位與至少一欄可見；CEO 待核版本則由 `seasonal-approval.ts` 提供活動／revision／送核時間／hash 的純搜尋與排序。`CorrectionHistoryTable`／`correction-history.ts` 只組合五個更正來源的唯讀歷史，不得承擔過帳或直接資料異動。帳號清單只選取資料，異動仍走 `/api/admin/accounts`；兩倉庫存只讀 `v_item_availability`；營運報表只讀 security-invoker views；原因碼保存只走 `maintain_procurement_difference_reason`；CEO 核准仍只走 revision/hash 重驗的 `review_seasonal_submission`。
