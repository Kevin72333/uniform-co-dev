...
Repository-local 最新完整驗證為：70 個 test files、284/284 tests 通過，`npm run lint`、`npm run typecheck`、`npm run build` 與 System Guide parser tests 全部成功；`git diff --check` 沒有內容錯誤。推送後仍需補正式站 read-only UI smoke。下一位 agent 不應把「找不到新的本機 TODO」或本機驗證成功解讀為正式上線已完成。
...
9. **共用管理清單**：參考 SPSV29 的大型清單操作後，以 `src/app/ManagementCatalogTable.tsx` 集中帳號、商品、組織、員工、兩倉庫存、營運報表、採購差異原因碼、更正歷史、CEO 待核版本、換季採購決策品項與換季需求登記的欄位顯示、密度、每頁筆數、資料範圍、排序表頭與首末頁導覽；`src/app/CorrectionHistoryTable.tsx` 以共用表格組合更正歷史的單號／狀態／差額／原因搜尋與狀態篩選；CEO 待核版本的搜尋／排序純規則在 `src/domain/seasonal-approval.ts`，採購品項的搜尋／排序純規則在 `src/domain/seasonal-procurement.ts`，需求登記的搜尋／排序純規則在 `src/domain/seasonal-demand.ts`；其他 page／column 規則在 `src/domain/management-catalog.ts`，業務篩選排序分別在各自 domain module。資料查詢、RLS、匯出稽核與 mutation 仍留在各業務 module；沒有受保護 bulk RPC 前，不得加入批次刪除／停用。維護規則見 `docs/architecture/management-catalog.md`。
...
- 管理清單 seam：`ManagementCatalogTable` 只接收已篩選排序的 rows、欄位定義及 page／sort callbacks；呼叫端保留查詢與 mutation。`management-catalog.ts` 的純 interface 必須固定 page clamp、核准 page size、固定欄位與至少一欄可見；CEO 待核版本則由 `seasonal-approval.ts` 提供活動／revision／送核時間／hash 的純搜尋與排序，換季採購決策品項由 `seasonal-procurement.ts` 提供品號／品名／數量／決策狀態的純搜尋與排序，換季需求登記由 `seasonal-demand.ts` 提供員工／品號／數量／更新時間的純搜尋與排序。`CorrectionHistoryTable`／`correction-history.ts` 只組合五個更正來源的唯讀歷史，不得承擔過帳或直接資料異動。帳號清單只選取資料，異動仍走 `/api/admin/accounts`；兩倉庫存只讀 `v_item_availability`；營運報表只讀 security-invoker views；原因碼保存只走 `maintain_procurement_difference_reason`；CEO 核准仍只走 revision/hash 重驗的 `review_seasonal_submission`，採購決策仍只走 `set_seasonal_procurement_line`；需求登記清單只讀目前活動可見列，選取後由原 panel 回填，保存仍只走 `upsert_seasonal_demand_line`。
