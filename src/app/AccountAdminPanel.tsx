...
export default function AccountAdminPanel() {
  ...
  return <section className="panel import-panel" aria-label="帳號角色與需求窗口管理">
    ...
    {selected ? <>
      ...
      <div className="button-row"><button className="secondary-button" type="button" onClick={() => void setRoleValue()} disabled={busy}>{roleEnabled ? "啟用角色" : "停用角色"}</button><button className="secondary-button" type="button" onClick={() => void setStatus()} disabled={busy}>{selected.is_active ? "停用登入與帳號" : "重新啟用帳號"}</button><button className="secondary-button" type="button" onClick={() => void rebindAuth()} disabled={busy || (!unbindAuth && !newAuthUserId.trim())}>{unbindAuth ? "解除 Auth 綁定" : "重設 Auth 綁定"}</button><button className="secondary-button" type="button" onClick={() => void deleteAccount()} disabled={busy || selected.id === selectedId && !selected.is_active}>刪除登入身份</button></div>
