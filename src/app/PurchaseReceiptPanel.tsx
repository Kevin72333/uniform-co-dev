...
export default function PurchaseReceiptPanel() {
  ...
  return <section className="panel import-panel" aria-label="採購入庫">
    ...
    <div className="form-grid">
      ...
      <label className="field"><span>到貨日期</span><input type="date" value={receivedOn} onChange={(event) => { resetOperationKeys(); setReceivedOn(event.target.value); }} disabled={busy || Boolean(receipt)} /></label>
      <label className="field"><span>到貨量</span><input type="number" min={1} value={deliveredQuantity} onChange={(event) => { resetOperationKeys(); setDeliveredQuantity(Math.max(0, Number(event.target.value) || 0)); }} disabled={busy || Boolean(receipt)} /></label>
      <label className="field"><span>合格量</span><input type="number" min={0} value={acceptedQuantity} onChange={(event) => { resetOperationKeys(); setAcceptedQuantity(Math.max(0, Number(event.target.value) || 0)); }} disabled={busy || Boolean(receipt)} /></label>
      <label className="field"><span>拒收量</span><input type="number" min={0} value={rejectedQuantity} onChange={(event) => { resetOperationKeys(); setRejectedQuantity(Math.max(0, Number(event.target.value) || 0)); }} disabled={busy || Boolean(receipt)} /></label>
