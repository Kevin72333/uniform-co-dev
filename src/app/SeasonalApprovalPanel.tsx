...
export default function SeasonalApprovalPanel() {
  ...
  async function review() {
    ...
    const { error } = await client.rpc("review_seasonal_submission", {
      ...
      p_reason: reason.trim() || null, p_idempotency_key: `REVIEW-SEASONAL-${crypto.randomUUID()}`,
    ...
    else { setMessage(decision === "APPROVE" ? "已核准，採購可依核准量進行 MOQ 判斷。" : "已退回 HR 修正，退回理由已保留。"); setSubmissions((rows) => rows.filter((row) => row.id !== submission.id)); setReason(""); }
    setBusy(false);
  }
  ...
  return <section className="panel import-panel" aria-label="CEO 換季審核">
    ...
    <div className="button-row"><button className="primary-button" type="button" onClick={() => void review()} disabled={busy || !selected}>{busy ? "送出中…" : decision === "APPROVE" ? "核准此版本" : "退回 HR"}</button></div>
