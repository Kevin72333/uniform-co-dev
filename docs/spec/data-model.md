...
| 實體 | 主鍵／唯一鍵 | 關鍵欄位與關係 |
|---|---|---|
| app_accounts | id uuid PK；auth_user_id uuid UNIQUE NULL；login_name case-insensitive UNIQUE NULL | login_name、display_name、email_snapshot、is_active。login_name 是新帳號必填的登入名稱；email_snapshot 是選填聯絡資料，不作登入用途。既有 email 登入帳號轉換前可暫時沒有 login_name。id 是可還原且不可變的業務帳號識別；auth_user_id 只用來綁定目前 Supabase Auth 身分，不作任何業務 actor FK。 |
