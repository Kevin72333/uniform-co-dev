...
`ManagementCatalogTable` 是帳號、商品、組織、員工、兩倉庫存、營運報表、採購差異原因碼、更正歷史與 CEO 待核版本共用的管理清單 module。它把欄位顯示、資料密度、每頁筆數、目前範圍、首末頁導覽、排序表頭與空狀態集中在同一個 interface；各業務清單只提供欄位定義、已篩選排序的資料列及逐列操作。
